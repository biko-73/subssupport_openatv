...
def search_tvshow(tvshow, season, episode, languages, filename):
    ...
    content, response_url = requests.get(url,headers=HDR,allow_redirects=True).text
...
def search_subtitles(file_original_path, title, tvshow, year, season, episode, set_temp, rar, lang1, lang2, lang3, stack):  # standard input
    log(__name__, "%s Search_subtitles = '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s'" %
         (debug_pretext, file_original_path, title, tvshow, year, severify=Trueason, episode, set_temp, rar, lang1, lang2, lang3, stack))
    if lang1 == 'Farsi':
        lang1 = 'Persian'
    if lang2 == 'Farsi':
        lang2 = 'Persian'
...
def download_subtitles (subtitles_list, pos, zip_subs, tmp_sub_dir, sub_folder, session_id):  # standard input
    ...
    if (success == True):
        ...
        response = requests.get(downloadlink,data=postparams,headers=HDRDL,allow_redirects=True) 
